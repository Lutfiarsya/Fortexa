...
const RegisterPage = () => {
    ...
    return (
        <div className="h-screen font-['Poppins'] w-screen flex flex-col items-center justify-center">
            ...
            {/* form */}
            <div className="flex flex-col w-[90vw] justify-center gap-6 h-[60vh] px-4">
                <div>
                    <h2 className="font-semibold text-2xl">Welcome</h2>
                    <p className="font-regular italic text-xs  text-gray-500">Sign up to your money manager</p>
                </div>
                ...
                <div className="flex flex-col relative gap-2">
                    ...
                    <input
                    style={{width: widthResponsive(317), height: heightResponsive(48)}} 
                    type={visible ? `text` : `password`} className="p-2 rounded-md text-base bg-[#F2F2F2] border-1 border-gray-300 placeholder:text-xs"  placeholder="Confirm password" required/>
                    <div onClick={() => setVisible(!visible)} className="absolute w-10 right-6 items-center flex justify-center bottom-0 h-[50%] flex">
                    { visible ? <Eye size={20}/> : <EyeOff size={20}/> }
                    </div>
                </div>
            ...
            <div className="flex flex-col justify-start h-[10vh] items-center w-full gap-4">

                <div className="flex flex-row items-center justify-center gap-2">
                    <div className="bg-black w-20 h-[0.8px]"></div>
                    <h2 className="text-[10px]">Or sign up with</h2>
                    <div className="bg-black w-20 h-[0.8px]"></div>
                </div>
                ...
                <h2 className="text-base">Already have an account?<span onClick={() => navigate('/login')} className="ml-2 hover:cursor-pointer text-[var(--primary-color)]">Login</span></h2>
            </div>
        </div>
    )
}
